...
tau_grid  =  c(seq(1.2,3,length = 15),100)
...
for(T_ind in 1: length(T_grid) )
{
  ...
  for(iter in 1:NMC)
  {
    ...
    for(t in 1:T)
    {
      if( t<= v[1])
      {
        y[t,1:N[t]]  =  sample(x= (1:length(background_train))/length(background_train),size= N[t], prob = f0[1,]) 
      }
      if(K >1)
      {
        for(j in 1:(K-1))
        {
          if(  t >  v[j]   &&  t <=  v[j+1])
          {
            y[t,1:N[t]]  =  sample(x= (1:length(background_train))/length(background_train),size=  N[t], prob = f0[j+1,]) 
          }
        }
      }
      if(t > v[K])
      {
        y[t,1:N[t]]  =  sample(x= (1:length(background_train))/length(background_train),size= N[t], prob = f0[K+1,]) 
      }
    }###############
    y_raw =  c()
